<template>
  <article class="w-full p-6 mb-2 bg-gray-100" data-testid="review-item">
    ...
    <header>
        <div class="flex justify-between w-full">
            <div class="flex items-center pr-2 pb-2 text-base text-gray-600">
                <SfRating :value="reviewGetters.getReviewRating(reviewItem) ?? undefined" :max="5" size="sm" class="mr-2 mb-1 !text-black" />
                {{ $d(new Date(reviewGetters.getReviewDate(reviewItem))) }}
            </div>
            <div class="shrink-0 text-base truncate text-black mb-2">
                <span data-testid="review-item-authorName">{{ reviewAuthor }}</span>
                <span v-if="verifiedPurchase" class="text-green-800 ml-2">
                    <SfIconCheck size="xs" class="mr-1" />
                    {{ t('review.verifiedPurchase') }}
                </span>
            </div>
        </div>
        <p class="text-2xs mb-2" data-testid="review-item-title">{{ reviewGetters.getReviewTitle(reviewItem) }}</p>
    </header>

    <p class="text-sm text-gray-600">{{ reviewGetters.getReviewMessage(reviewItem) }}</p>
